...
export default (props: FromTootToCard) => {
	const { card, width, withoutLine } = props
    if(!card) return null
	return (
		<TouchableOpacity style={!withoutLine ? { borderWidth: 1, borderRadius: 10, padding: 5 } : {}} onPress={async () => await WebBrowser.openBrowserAsync(card.url)}>
